...
exports.submitContestCode = async (req, res) => {
  try {
    ...
    // Enforce allowed languages only
    ...
    for (const result of testResults) {
      if (result.status.id === 3) {
        ...
        status = "Wrong Answer";
        errorMessage =
          result.compile_output || result.stderr || "Test case failed";
        break;
      }
    }
        ...
        if (solvedProblems.length === totalProblemsInContest) {
          ...
          if (!user.contestsCompleted.some((c) => c.toString() === contestId)) {
            ...
            const lastCompletion = user.lastContestCompletion
              ? new Date(user.lastContestCompletion)
              : null;
    ...
    return res.status(500).json({
      success: false,
      message:
        error.message || "Internal server error while processing submission",
    });
  }
};
...
exports.runContestCode = async (req, res) => {
  try {
    ...
    const allowedLanguages = new Set(["c++", "java", "python"]);
    if (!normalizedLanguage || !allowedLanguages.has(normalizedLanguage)) {
      return res.status(400).json({
        success: false,
