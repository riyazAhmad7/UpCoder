...
const submitCode = async (req, res) => {
  try {
    ...
    // Normalize and whitelist languages (only allow c++, java, python)
    language = String(language).toLowerCase();
    if (language === "cpp") language = "c++";
    const allowedLanguages = new Set(["c++", "java", "python"]);
    if (!allowedLanguages.has(language)) {
      return res.status(400).send("Unsupported programming language");
    }
    ...
    if (!problem.hiddenTestCases || problem.hiddenTestCases.length === 0) {
      ...
      return res.status(400).json({
        success: false,
        message:
          "This problem has no hidden test cases and cannot be submitted.",
      });
    }
    ...
    if (referenceSolution && startCode) {
      finalCode = referenceSolution.completeCode.replace(
    ...
    console.log(
      "submitCode: Checking if problem is already solved. Status:",
      status
    );
    console.log(
      "submitCode: User's problemSolved array:",
    ...
    if (status === "Accepted") {
      try {
        ...
        if (updatedUser) {
          console.log(
            "submitCode: User problemSolved array updated successfully."
          );
          const io = getIO();
          if (io) {
            io.to(userId.toString()).emit("userStatsUpdate", { userId });
            console.log(
              `submitCode: Emitted userStatsUpdate event to user ${userId}.`
            );
          }
        } else {
        ...
        console.error(
          "submitCode: Error updating user's solved problems:",
          userUpdateError
        );
