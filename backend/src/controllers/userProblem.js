  ...
  try {
    ...
    for (const { language, completeCode } of referenceSolution) {
      if (!language || !completeCode) {
        return res
          .status(400)
          .send("Missing language or completeCode in referenceSolution.");
      }
      ...
      if (!submitResult || !Array.isArray(submitResult)) {
        return res
          .status(500)
          .send("Judge0 submission failed or no result returned.");
      }
      ...
      for (const test of testResult) {
        if (test.status_id != 3)
          return res
            .status(400)
            .send("Error Occured Status_code is not equal to 3 ");
      }
...
const getProblemById = async (req, res) => {
  try {
    ...
    const getProblem = await Problem.findById(id).select(
      "title description difficulty tags visibleTestCases startCode referenceSolution constraints"
    );
...
const getAllProblems = async (req, res) => {
  try {
    const getProblems = await Problem.find({ visibility: "public" }).select(
      "_id title tags difficulty companyAsked"
    );
...
const problemsSolvedByUser = async (req, res) => {
  try {
    const userId = req.result._id;
    console.log(
      "problemsSolvedByUser: Fetching solved problems for userId:",
      userId
    );
    ...
    if (rawUser) {
      console.log(
        "problemsSolvedByUser: Raw problemSolved array:",
        rawUser.problemSolved.map((id) => id.toString())
      );
    }

    // Now get the populated user with only public problems
    const user = await User.findById(userId).populate({
      path: "problemSolved",
      select: "_id title difficulty tags companyAsked",
      match: { visibility: "public" }, // Only populate public problems
    });

    if (user && user.problemSolved) {
      console.log(
        "problemsSolvedByUser: Populated problemSolved array length:",
        user.problemSolved.length
      );
      console.log(
        "problemsSolvedByUser: First few problems:",
        user.problemSolved
          .slice(0, 3)
    ...
    console.log(
      `problemsSolvedByUser: Found ${user.problemSolved.length} solved public problems`
    );
    res.status(200).json({
      success: true,
...
const getProfileAllProblems = async (req, res) => {
  try {
    ...
    // Get only public problems
    const allProblems = await Problem.find({ visibility: "public" }).select(
      "_id title difficulty tags companyAsked"
    );
    ...
    const attemptingProblemIds = [
      ...new Set(attemptingSubmissions.map((s) => s.problemId)),
    ];
    ...
    const problemsWithStatus = allProblems.map((problem) => {
      ...
      const isAttempting =
        !isSolved && attemptingProblemIds.some((id) => id.equals(problem._id));
      return {
        ...problem.toObject(),
        isSolved,
        isAttempting,
