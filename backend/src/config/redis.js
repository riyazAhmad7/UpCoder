...
const redisClient = redis.createClient({
  ...
  retry_strategy: function (options) {
    if (options.error && options.error.code === "ECONNREFUSED") {
      ...
      console.error(
        "Redis connection refused. Please check if Redis server is running."
      );
...
const redisWrapper = {
  ...
  connect: async () =>
    safeRedisOperation(redisClient.connect.bind(redisClient)),
  set: async (key, value) =>
    safeRedisOperation(redisClient.set.bind(redisClient), key, value),
  get: async (key) =>
    safeRedisOperation(redisClient.get.bind(redisClient), key),
  del: async (key) =>
    safeRedisOperation(redisClient.del.bind(redisClient), key),
  exists: async (key) =>
    safeRedisOperation(redisClient.exists.bind(redisClient), key),
  expire: async (key, seconds) =>
    safeRedisOperation(redisClient.expire.bind(redisClient), key, seconds),
  expireAt: async (key, timestamp) =>
    safeRedisOperation(redisClient.expireAt.bind(redisClient), key, timestamp),
  zAdd: async (key, members) =>
    safeRedisOperation(redisClient.zAdd.bind(redisClient), key, members),
  zCard: async (key) =>
    safeRedisOperation(redisClient.zCard.bind(redisClient), key),
  zRemRangeByScore: async (key, min, max) =>
    safeRedisOperation(
      redisClient.zRemRangeByScore.bind(redisClient),
      key,
      min,
      max
    ),
