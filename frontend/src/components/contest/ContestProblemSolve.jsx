...
const ContestProblemSolve = () => {
  ...
  useEffect(() => {
    ...
    const calculateRemainingTime = () => {
      ...
      if (timeLeft <= 0) {
        ...
        // Redirect away from problem page once contest ends
        try {
          navigate(`/contest/${contestId}`);
        } catch (_) {}
        return null;
  ...
  const handleRunCode = async () => {
    // Block run if contest has ended
    if (remainingTime === "Contest Ended") {
      showToast("Contest ended. Redirecting to contest page.", "warning");
      navigate(`/contest/${contestId}`);
      return;
    }
    if (!code.trim()) {
  ...
  const handleSubmitCode = async (isAutoSubmit = false) => {
    // Block submit if contest has ended
    if (remainingTime === "Contest Ended") {
      if (!isAutoSubmit) {
        showToast("Contest ended. You can't submit anymore.", "error");
      }
      navigate(`/contest/${contestId}`);
      return;
    }
              ...
              <div className="prose prose-invert max-w-none">
                ...
                {problem.constraints && (
                  <div className="mt-6">
                    ...
                    <pre className="bg-gray-900 border border-orange-500/30 rounded-lg p-4 text-orange-300 whitespace-pre-wrap">
                      {problem.constraints}
                    </pre>
                  </div>
                )}
