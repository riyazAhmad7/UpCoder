  ...
  const fetchContestDetails = async () => {
    try {
      ...
      if (fetchedContest) {
        ...
        // Fetch problems for this contest only if active
        const now = new Date();
        if (now >= new Date(fetchedContest.startTime)) {
        ...
        <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl overflow-hidden mb-6">
          <div className="p-6 md:p-8">
            ...
            <div className="bg-gray-900/50 border border-gray-700/50 rounded-lg p-4 flex items-center justify-between">
              ...
              {(contestStatus === "active" || contestStatus === "upcoming") &&
                !hasCompleted &&
                !hasEntered && (
                  <button
                    onClick={async () => {
                      try {
                        ...
                        try {
                          ...
                          const { contest: updatedContest, userStatus } =
                            response.data;
                          setContest(updatedContest);
                          setParticipants(updatedContest.participants || []);
                          setHasCompleted(userStatus?.isCompleted || false);

                          // If contest is active after registration, navigate to first problem
                          const now = new Date();
                          if (
                            now >= new Date(updatedContest.startTime) &&
                            now <= new Date(updatedContest.endTime)
                          ) {
                            if (
                              updatedContest.problems &&
                              updatedContest.problems.length > 0
                            ) {
                              const firstProblemId =
                                typeof updatedContest.problems[0] === "object"
                                  ? updatedContest.problems[0]._id
                                  : updatedContest.problems[0];
                              navigate(
          ...
          <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl overflow-hidden">
            <div className="p-4 border-b border-gray-700/50 flex justify-between items-center">
              ...
              <div className="text-sm text-gray-400">
                {contestStatus === "upcoming" ? 0 : problems.length} Problems
              </div>
            </div>

            {contestStatus === "upcoming" || problems.length === 0 ? (
