...
const ProfileCard = () => {
  const { user, profile, profileLoading, profileError } = useSelector(
    (state) => state.auth
  );
  ...
  return (
    <div className="bg-dark-800 rounded-lg p-6 shadow-lg border-l-4 border-orange-primary">
      <div className="flex items-center space-x-4 mb-4">
        ...
        <div>
          ...
          <p className="text-gray-400">
            {displayData?.emailId || displayData?.email}
          </p>
        </div>
      </div>
      ...
      {profile?.problemStats && (
        <div className="mt-6">
          <h3 className="text-lg font-semibold mb-2 text-orange-primary">
            Problem Stats
          </h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-dark-700 p-3 rounded-lg">
