...
const CreateContestForm = ({ onSuccess }) => {
  ...
  useEffect(() => {
    // Set minimum datetime-local value to now (rounded to minutes)
    const now = new Date();
    now.setSeconds(0, 0);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        "Failed to create contest. Please check the form and try again."
      );
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-slate-400 mb-8 -mt-4">
          Set up challenges for your community
        </p>
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="p-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      min={nowIso}
                      className={`w-full p-4 bg-slate-700/50 border rounded-lg text-white focus:ring-2 focus:ring-orange-500/20 transition-all outline-none ${
                        dateError
              ...
              <motion.div variants={itemVariants} className="space-y-4">
                <div>
                  ...
                  {/* Problem Selection Warning */}
                  {form.problems.length === 0 && (
                    <motion.div
                      initial={{ opacity: 0, height: 0 }}
                      animate={{ opacity: 1, height: "auto" }}
                      exit={{ opacity: 0, height: 0 }}
                      className="mb-4 p-3 bg-orange-500/10 border border-orange-500/20 rounded-lg flex items-center gap-2"
                    >
            ...
            >
              ...
              <motion.button
                type="submit"
                disabled={isLoading || form.problems.length === 0}
                ...
                whileHover={form.problems.length > 0 ? { scale: 1.02 } : {}}
                whileTap={form.problems.length > 0 ? { scale: 0.98 } : {}}
              >
                {isLoading ? (
                  <div className="flex items-center gap-2">
