...
const UpdateProblem = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    startCode: [{ language: "python", code: "" }],
  ...
  const fetchProblems = async () => {
    setLoading(true);
    try {
      const response = await axiosClient.get("/contest/problems");
  ...
  const filteredProblems = problems.filter(
    (problem) =>
      ...
      problem.visibility?.toLowerCase().includes(searchTerm.toLowerCase()) ||
